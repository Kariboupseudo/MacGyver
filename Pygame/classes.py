...
class Niveau:
    """Niveau: This class build an object containing a maze
    in which it's possible to select a random position and then
    validate it in order to place items on a proper position."""
    ...
    def random_pos_items(self):
        ...
        for i in self.items:  # slicing objets in self.structure
            self.structure[i.poscase_y] = self.structure[i.poscase_y]\
                [0:i.poscase_x] + i.display + self.structure[i.poscase_y]\
                [i.poscase_x+1:]

    def valid_pos(self, candidate_pos):
        """Check if the position of the item is not
        already taken by a previously created item"""
        if not self.items:
            answer = True
        else:
            answer = True
            for i, item in enumerate(self.items):
                if [candidate_pos[0], candidate_pos[1]] ==\
                        [item.poscase_x, item.poscase_y]:
                    answer = False
...
class Perso:
    """Perso : builds a character with positionning
    values both in cases and in pixels.
    This class needs the level structure and the item
    list to verify each character's move."""
            ...
            if self.poscase_x < (nb_sprite_cote - 2):
                ...
                for i in self.items:
                    if self.level.structure[self.poscase_y][self.poscase_x+1] \
                        ...
                        # Item is added in Character's inventory
                        self.poscase_x += 1
                        # Calculation of the character's new position in pixels
                        self.pospix_x = self.poscase_x * taille_sprite
                # Checking if landing position is the guardian/exit position
                if self.level.structure[self.poscase_y]\
                        [self.poscase_x+1] == 'G':
                    self.end()
                    # Checking if character's has 3 items in his inventory
            ...
            if self.poscase_y > 0:
                for i in self.items:
                    if self.level.structure[self.poscase_y-1][self.poscase_x]\
                        ...
                        self.poscase_y -= 1
                        self.pospix_y = self.poscase_y * taille_sprite
                if self.level.structure[self.poscase_y-1]\
                        [self.poscase_x] == 'G':
                    self.end()
                if self.level.structure[self.poscase_y-1]\
                        [self.poscase_x] != 'W':
